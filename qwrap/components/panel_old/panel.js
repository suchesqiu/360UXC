	...
	Panel.prototype = function () {
		...
		return {
			...
			_initPanel: function (op, config) {
				...
				if (!this._panel.id) this._panel.id = (this.id || 'FANFAN_PANEL_' +(_id_cnt ++));
				return this;
			},
			...
			setPanelRect: function (x, y, w, h) {
				...
				D.setInnerRect(this._panel, x, y, w, h);
			...
			detectPanel: function() {
				if (!this._panel || !D.g(this._panel.id)) {
					alert('很抱歉，弹出层无法正常使用。\n可能是由于浏览器开启了"浮动广告拦截功能"，请尝试关闭该功能后重试。\n\n如何关闭"浮动广告拦截功能"请参阅帮助中心FAQ。');
					this.dispose();
					return false;
				}
	...
	QW.ObjectH.mix(BaseLayer.prototype, function () {
		...
		return {
			...
			initDraggable: function () {
				...
				var drag = new QW.SimpleDrag({ oSrc : this._panel, oHdl : this._header, oProxy : this.dragProxy, withProxy : this.withDragProxy });
