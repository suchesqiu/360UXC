(function () {
	...
	*/
    ...
    var target = QW.EventH.getTarget,
		createElement = QW.DomU.createElement,
        addEventListener = QW.EventTargetH.addEventListener,
		NodeH = QW.NodeH,
    ...
    Tree.prototype = {
            ...
            try {
                if (links.length) {
                    if (this._highlightEl) {
                        try {
                            removeClass(this._highlightEl, "highlight");
                        } catch (ex) {};
                    }
                    var aEl = links[0];
                    aEl.focus();
        ...
        _renderItems: function (id, ignoreDataNull) {
            ...
            if (itemsData.length) {
                ...
                for (var i = 0; i < itemsData.length; i++) {
					...
					var last = (i == itemsData.length - 1)? 'folder-last ' : '';
                    if (d[0].toLowerCase().indexOf('folder') > -1) {
                        html[i] = '<li id="' + this._idPre + d[1] + '" class="folder-closed '+last + d[0] + '"><span class="folder-img-closed ' + d[0] + '">&nbsp;</span><div class="node-ctn">' + str + '</div><ul style="display:none"></ul></li>';
                    } else {
                        html[i] = '<li id="' + this._idPre + d[1] + '" class="folder-closed '+last + d[0] + '"><div class="node-ctn">' + str + '</div></li>';
						html[i] = '<li id="' + this._idPre + d[1] + '" class="folder-closed ' + d[0] + '"><span class="folder-img-bottom ' + d[0] + '">&nbsp;</span><div class="node-ctn">' + str + '</div></li>';
						html[itemsData.length-1] = '<li id="' + this._idPre + d[1] + '" class="folder-closed ' + d[0] + '"><span class="folder-img-last ' + d[0] + '">&nbsp;</span><div class="node-ctn">' + str + '</div></li>';	
                    }
                }
			...
			var fLast = W('.folder-last');
			fLast.firstChild('ul').addClass('folder-ul-lst');
			fLast.firstChild('span').addClass('folder-span-lst');
        ...
        render: function () {
            ...
            addEventListener(treeWrap, "mouseover", function (e) {
                me._mouseover(e);
            });
            addEventListener(treeWrap, "mouseout", function (e) {
                me._mouseout(e);
            });
            addEventListener(treeWrap, "click", function (e) {
                me._click(e);
            });
            me.openFolder(d[1]);
        }
